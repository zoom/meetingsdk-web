...
| `obfToken` | [Zoom On Behalf Token (OBF Token)](https://developers.zoom.us/docs/meeting-sdk/auth/#join-meetings-and-webinars-with-an-obf-token) |

> **Note:** `zak` and `obfToken` are mutually exclusive — use only one or the other. Use `zak` when starting a meeting as host; use `obfToken` when joining on behalf of another user.
